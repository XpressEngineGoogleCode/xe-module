...
class pointsendModel extends pointsend
{
	...
	function getGrantInfo()
	{
		...
		if(is_null($module_config))
		{
			...
			$module_config->grants = $config->grants ? explode('|@|',$config->grants) : array();
		}
		...
		if(is_null($config))
		{
			...
			if($logged_info)
			{
				foreach($config as $key => $val)
				{
					...
					$fee = $val < 1 ? 0 : $val;
					$fee = $val > 99 ? 99 : $val;
					$config->fee_[$matches[1]] = $fee;

					if(!$is_admin)
					{
						$tmp_group[$matches[1]] = $matches[1];
						$tmp_fee[$matches[1]] = $fee;
					}
				}
				...
				if(!$is_admin)
				{
					if(count($tmp_group) && is_array($logged_info->group_list) && count($logged_info->group_list))
					{
						...
						{
							if(array_key_exists($key, $tmp_group))
							{
								$config->fee = $tmp_fee[$key];
								break;
							}
						}
					}
